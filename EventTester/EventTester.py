...
class EventServer:
    ...
    def decode_header(self, hdr):
        h = {}
        h['NWORDS'] = int.from_bytes(hdr[0x00:0x02], byteorder='little')
        h['VERSION'] = hdr[3:1:-1].decode()
        h['EVENT_NUM'] = int.from_bytes(hdr[0x04:0x08], byteorder='little')
        h['EVENT_TIME'] = int.from_bytes(hdr[0x08:0x0C], byteorder='little')
        h['LAST_PPS'] = int.from_bytes(hdr[0x0C:0x10], byteorder='little')
        h['LLAST_PPS'] = int.from_bytes(hdr[0x10:0x14], byteorder='little')
        meta = list(hdr[0x18:0x38])
        tmeta = meta[7:32:8]
        del meta[7:32:8]
        h['META_SURF'] = meta
        h['META_TURF'] = tmeta
        h['RUNCFG'] = int.from_bytes(hdr[0x7C:0x7E], byteorder='little')
        h['SURFWORDS'] = int.from_bytes(hdr[0x7E:0x80], byteorder='little')
        return h

    def print_header(self, hdr):
        h = self.decode_header(hdr)
        for k in h:
            print(f'{k} : {h[k]}')
    ...
    def __ack(self, msg, verbose=False):
        ...
        if verbose:
            ...
            print(f'ack response: {tohex(rmsg, s=" ")}')
